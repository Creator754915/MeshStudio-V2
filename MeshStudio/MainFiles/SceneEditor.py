...
from MeshStudio.MainFiles.AddEntity import AddEntity
from MeshStudio.MainFiles.NavBar import NavBar
from MeshStudio.MainFiles.SideBarTools import SideBarTools
from MeshStudio.MainFiles.WorldDirectionBox import DirectionBox
from MeshStudio.MainFiles.WorldGrid import WorldGrid
from MeshStudio.Prefabs.additive_function import MultiFunctionCaller, TextToVar
from MeshStudio.MainFiles.Gizmo.GizmoManager import GizmoManager
from MeshStudio.MainFiles.ColorMenu import ColorMenu
...
class SceneEditor(Entity):
    def __init__(self, ProjectName, ProjectDescription, ParentProjectEditor=scene, **kwargs):
        ...
        self.SideBarTopParentEntity = Entity(parent=self.UniversalParentEntity, model="cube",
                                             position=window.top_right - Vec2(0.12, 0), scale=Vec3(0.45, 0.6, 2),
                                             color=color.gray)
        self.SideBarTopSlideHandler = Button(name="GlobalEntityInfo", parent=self.SideBarTopParentEntity, model="cube",
                                             radius=0, visible_self=False,
                                             y=-0.22, z=-200)
        ...
        self.SideBarTools = SideBarTools()
        ...
        self.DirectionEntity = DirectionBox(parent=camera.ui, camera=self.EditorCamera, enabled=True,
                                            ...
                                            scale=.018,
                                            position=window.top_right - Vec2(.45, .12))

        self.AddEntityInScene()

    def AddEntityInScene(self):
        self.WorldItems.append(
            #AddEntity(color=color.white))
            Cube(name=f"item_{len(self.WorldItems)}", parent=scene, scale=(1, 1, 1), texture="white_cube",
