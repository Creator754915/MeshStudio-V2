...
from MeshStudio.ShaderEditor.OpenFile import AskSaveFile
...
from MeshStudio.Prefabs.additive_function import RecursivePerformer, ReplaceValue


class FileMenu(Entity):
    '''Used by CodeEditorPython that Operates file system and data saving into it'''

    def __init__(self, ProjectName, Path, parent, CodeEditorEntity: TextField, ShowInstructionFunc, OnFileAdded,
                 queue=1, z=0, UdSrc=[], **kwargs) -> None:
        ...
        self.AddFileButton: Button = Button(name="AddFileButton", parent=self.TobBarParentEntity, text="New\nFile",
                                            position=Vec3(-0.43, 0.42, z), rotation=Vec3(0, 0, 0),
                                            scale=Vec3(0.11, 0.12, 1), render_queue=queue,
                                            on_click=self.RegisterFileToEdit)
        self.AddFolderButton: Button = Button(name="FolderButton", parent=self.TobBarParentEntity, text="New\nFolder",
                                              scale=Vec3(0.11, 0.12, 1), position=Vec3(-0.3, 0.42, z),
                                              render_queue=queue)
        self.SearchButton: Button = Button(name="SearchButton", parent=self.TobBarParentEntity, text="Search",
                                           scale=Vec3(0.11, 0.12, 1), position=Vec3(-0.17, 0.42, z), render_queue=queue)

        self.FilesAndButtonSeparator: Entity = Entity(name="separator", parent=self.UniversalParentEntity, model="line",
                                                      position=(window.left + Vec2(0.89, 0.33)))
        ...
        def InputFieldToTextFile(Field: InputField):
            if held_keys["enter"]:
                if Field.text in self.CurrentFileName:
                    self.ShowInstructionFunc(Str="Cannot make another file when a file already exists with that name",
                                             Title="Error")
                else:
                    self.BackUpUdSrc.append({f"{Field.text}.glsl": ""})
                    self.CurrentFileName.append(f"{Field.text}.glsl")

                    self.AddFileToEdit(f"{Field.text}.glsl", len(self.BackUpUdSrc) - 1)
                    self.OnFileAdded()

            destroy(Field)

        self._Temp2 = InputField(y=-self._Temp / 10, parent=self.FileSystemMenuParentEntity, scale_y=.1,
                                 render_queue=self.UniversalParentEntity.render_queue, submit_on=["enter", "escape"])
    ...
    def AddFileToEdit(self, Name: str, CurrentIndex: int) -> None:
        ...
        self._Temp = len(self.TotalProjectFiles)
        self.TotalProjectFiles.append(
            Button(text=Name, y=-self._Temp / 10, parent=self.FileSystemMenuParentEntity, scale_y=.1,
                   render_queue=self.UniversalParentEntity.render_queue))
    ...
    def JumpFiles(self, FileName, DictIndex, ToJumpFileButton) -> None:
        ...
        if self.CurrentEditingFileButton is not None:
            ...
            ToJumpFileButton.color = color.rgba(0, 0, 255, 150)
    ...
    def SaveCurrentFile(self) -> None:
        ...
        if self.CurrentEditingFileButton is not None:
            self.BackUpUdSrc[self.WorkingFileLocation[0]][self.WorkingFileLocation[1]] = self.CodeEditorTextField.text
            print(self.BackUpUdSrc[self.WorkingFileLocation[0]][self.WorkingFileLocation[1]])

            AskSaveFile(FileName=self.WorkingFileLocation[1], Data=self.CodeEditorTextField.text)
